...
//	public static boolean warnPermBan;
//	public static int warnBanTime;
//	public static String warnBanTimeUnit;
	...
	public static void addPlayer(String playername, String uuid) {
		executor.execute(new Runnable() {
			@Override
			public void run() {
				String qry = "INSERT INTO " + MySQL.TABEL_WARNEDPLAYERS + " (playername, uuid, warns) VALUES (?, ?, ?)";
				try {
					PreparedStatement stmt = MySQL.con.prepareStatement(qry);
	...
	public static void addWarn(String uuid, int warns) {
		executor.execute(new Runnable() {
			@Override
			public void run() {
				int nowwarns = WarnManager.getWarns(uuid);
				nowwarns += warns;
